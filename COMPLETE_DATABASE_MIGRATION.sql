...
-- This single file contains ALL necessary database changes
-- Run this entire script in Supabase SQL Editor to fix all issues

-- ============================================================================
-- PART 1: CREATE DOCUMENTS STORAGE BUCKET (Fixes screenshot viewing error)
-- ============================================================================
...
-- ============================================================================
-- PART 2: UPDATE ENROLLMENT FORM FIELDS (Sync with frontend changes)
-- ============================================================================
...
-- Insert any missing default fields for existing courses
...
SELECT 'STORAGE BUCKET CHECK' as check_type, 
       CASE WHEN EXISTS(SELECT 1 FROM storage.buckets WHERE name = 'documents') 
            THEN 'SUCCESS: documents bucket exists' 
            ELSE 'ERROR: documents bucket missing' 
       END as result;

-- Check country field has 71 options
SELECT 'COUNTRY OPTIONS CHECK' as check_type,
       'SUCCESS: ' || jsonb_array_length(field_options) || ' countries available' as result
FROM public.enrollment_form_fields 
WHERE field_name = 'country' 
LIMIT 1;

-- Check state and city are required
SELECT 'LOCATION FIELDS CHECK' as check_type,
       string_agg(field_name || ': ' || field_label || ' (required=' || is_required || ')', ', ') as result
FROM public.enrollment_form_fields 
WHERE field_name IN ('state', 'city');

-- Check academic status options
SELECT 'ACADEMIC STATUS CHECK' as check_type,
       'SUCCESS: ' || jsonb_array_length(field_options) || ' options available' as result
FROM public.enrollment_form_fields 
WHERE field_name = 'academic_status' 
LIMIT 1;
