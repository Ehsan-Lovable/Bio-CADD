...
-- 3. Create secure role update function to prevent privilege escalation
...
AS $$
  ...
  SELECT 
    p.id as user_id,
    'rapid_profile_updates' as activity_type,
    COUNT(*) as count,
    '1 hour' as time_window
  FROM profiles p
...
REVOKE ALL ON FUNCTION public.secure_update_user_role FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.secure_update_user_role TO authenticated;
